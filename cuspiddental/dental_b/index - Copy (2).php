		...
		//	echo "<tr><td class='headcol'>$hour:$minute $am_pm</td>";
			...
			while($i2 < $n2){
				...
				if(!$appointment_exists){
					echo "<td>";
					echo "<form action='' method='POST'  name='' id='' class='patient_form'>";
					echo "<input type='hidden' name='token_tdone_appointment'  value=$_SESSION[token_tdone_appointment] />";	
					$val2=$encrypt->encrypt("$rank#$minute#$surgery_id_array[$i2]#$appointment_date");
					echo "<input name=ninye type=hidden value='$val2' /><input type=submit value='$hour:$minute $am_pm $surgery_name_array[$i2]' class=' button_style' /></td>";
					echo "</form>";
					echo "</td>";
				}					
				$i2++;
...
elseif(isset($_POST['tdone_appointment'])){
	...
	<div class='grid-35 select_date'>
		<div class='grid-60'><label for="" class="label">Select appointment Date</label></div>
		<div class='grid-40'><input type=text id=appointment_date class='date_picker_no_past tdone_appointment_date'/></div>
	</div>
	...
	<form action="#prescribe_drug" method="POST"  name="" id="" class='tab_form patient_form'>
		...
		<div class='presc_container grid-100 no_padding'>
			<div class='grid-100 highlight_on_hover1 no_padding'>
				...
				<div class=' grid-25'><textarea disabled class='drug_details' width='100%' name='details[]' ></textarea></div>
				<div class=grid-15>
...
elseif(isset($_POST['add_drug']) and $_POST['add_drug']!='' and userHasRole($pdo,20)){
	...
	?><div class='grid-100 highlight_on_hover1 no_padding'>
		...
		<div class=grid-25><textarea disabled width='100%' class='drug_details' name='details[]' ></textarea></div>
		<div class=grid-15>
			<select name='presc_type[]' disabled class='drug_presc_type' ><option></option>
				<?php
					$self=$encrypt->encrypt("2");//self
...
elseif(isset($_POST['invoice_disp_num_deleted']) and $_POST['invoice_disp_num_deleted']!='' ){
	...
	<div class='grid-100 no_padding '> <?php
		...
		echo "<div class='invoice_view_table'>";
			...
			foreach($s as $row){
				$amount=html("$row[amount]");
				echo "<div class=invoice_view_row><div class='inv_view_80 '>CO-PAYMENT</div><div class='inv_view_20 '>(xx".number_format($amount,2).")</div></div>";
				$total = $total  - $amount;
			}
			...
			while($i < $n){
				...
				//ensure all fields are correctly set
					...
					$sql="select a.name, b.name as insurer_name, insurer_id, co_pay_type, value, pre_auth_needed, smart_needed, start_cover, end_cover,
						cover_type, cover_limit, insured from covered_company a left join insurance_company b on a.insurer_id=b.id
		...
		try{
			$pdo->beginTransaction();
				while($i < $n){
					...
					if($drug_in_stock < $quantity_sold){
					...
					$sql="insert into prescriptions set when_added=now(), drug_id=:drug_id, details=:details,pay_type=:pay_type,
							...
							prescription_id=:prescription_id, quantity=:quantity_sold";
