...
function idle__time_check(){
			//alert('x');
			var xdf='xdf';
			var form_data = {xdf: xdf}
			$.ajax({
				type: "POST",
				url: "dental_b/",
				data: form_data,
				error: function() {
					alert(" An Error occured, unable to log user out");
					e.preventDefault();
				},
				success: function(data) {
					//alert('data is ' + data);
					var r1 = data.split('minutes');
					if( r1.length == 2){
...
function  mainmenu(){
window.setInterval(idle__time_check, 900000);
...
//get total for amount of invoice paid in invoice payments
$('.invoice_payment_amount').mouseleave(function(){
	var total_payment = 0;
	var false_number=false;
	$('.invoice_payment_amount').each(function() {
				if($(this).val() != ''){
					//check if numeric
					
					var var1 = $(this).val();
					var1 = remove_commas(var1);
					if(IsNumeric_jq(var1)){
						total_payment = parseFloat(total_payment) + parseFloat(var1) ;
					}
					else{
						false_number=true;
	...
	//alert(addCommas(total_payment) + ' ' + false_number);
	if(!false_number && total_payment > 0 ){$('#total_payment').val(addCommas(total_payment));}
	else if(false_number){$('#total_payment').val('Invalid Number');}
});
...
$('.div_shower31').on('click', '.change_pt_details', function(e){
	var var1=$(this).parent().prev().children('input').val();
	//alert('hhh ' + var1);
	
	//$('.div_shower31').parent().find('.patient_form').submit();
	//$('#patient_contacts_form', window.parent.document).submit();
	//$(window.parent.document).find("#patient_contacts_form").submit();
//	window.opener.$(".patient_form").submit();
	var form_name=window.parent.$('.patient_form2');
	$('.div_shower31').dialog('close');
	$('<input />').attr('type', 'password')
            .attr('name', "admin_p")
			.attr('id', "admin_p")
            .attr('value', var1)
            .appendTo(form_name);
			var form_name = document.employer_form;
			window.parent.$('.patient_form2').submit();
			$('.div_shower31').dialog('close');
	
			
});	

//append details to corporate edit
$('#edit_ins_cover').on('click', '.changed_patient_type', function(e){
	var var1=$(this).parent().prev().children('input').val();
	//alert('hhh ' + var1);
	...
	//$(window.parent.document).find("#patient_contacts_form").submit();
//	window.opener.$(".patient_form").submit();
	var form_name=window.parent.$('.dialog_form');
	//$('#edit_ins_cover').dialog('close');
	$('<input />').attr('type', 'password')
            .attr('name', "admin_p")
			.attr('id', "admin_p")
            .attr('value', var1)
			.attr('class', 'corprate_swap_password')
            .appendTo(form_name);
			//.hide();
			//var form_name = document.employer_form;
			window.parent.$('.dialog_form').submit();
			//form_name.remove('.corprate_swap_password');//$('#edit_ins_cover').dialog('close');
			
	
			
});	

//this will take someone to patient contacts from surgery reports
$('.goto_pt_contact2').click(function(e){
	e.preventDefault();
	var goto_pt_contact2 = $(this).parent().next().text();
		var form_data = {goto_pt_contact2: goto_pt_contact2}
		$.ajax({
		type: "POST",
		url: "dental_b/",
		data: form_data,
		...
		//alert(data);
			var x = data.split('#');
			if(x[0] == 'good' && x[1] == 'patient_contacts'  ){
				window.location = "?id=patient#patient_contacts" ;
			}
		},
		complete: function() {
		}
...
//this will take someone to tdone from surgery allocations
$('.goto_tdone2').click(function(e){
	e.preventDefault();
	var goto_tdone2 = $(this).text();
		var form_data = {goto_tdone2: goto_tdone2}
		$.ajax({
		type: "POST",
		url: "dental_b/",
		data: form_data,
		...
		//alert(data);
			var x = data.split('#');
			if(x[0] == 'good' && x[1] == 'treatment_done'  ){
				window.location = "?id=patient#treatment_done" ;
			}
		},
		complete: function() {
		}
...
//this will take someone to patient contacts from allocations
$('.goto_pt_contact').click(function(e){
	e.preventDefault();
	var goto_pt_contact = $(this).prev().val();
		var form_data = {goto_pt_contact: goto_pt_contact}
		$.ajax({
		type: "POST",
		url: "dental_b/",
		data: form_data,
...
//this will take someone to tdone from allocations
$('.goto_tdone').click(function(e){
	e.preventDefault();
	var goto_tdone = $(this).prev().val();
		var form_data = {goto_tdone: goto_tdone}
		$.ajax({
		type: "POST",
		url: "dental_b/",
		data: form_data,
...
//show invoice from tdone admin invoice approve
$('#tdone_invoice_admin_approve').on('click', '.show_invoice_new2', function(e){
	e.preventDefault();
	var invoice_disp_num2 = $(this).attr('href');
	//alert(invoice_disp_num);
	var invoice_val = $(this).text();
	var encoded2 = 'yes';
		...
		title: 'INVOICE: ' + invoice_val,
		height: height_y,
		width: width_x,
		modal: true}).load('dental_b/', {'invoice_disp_num2': invoice_disp_num2 , 'encoded2': encoded2});
});

//show invoice from partially authorised  invoice approve
$('.show_invoice_new2').click(function(e){
	e.preventDefault();
	var invoice_disp_num2 = $(this).attr('href');
	//alert(invoice_disp_num);
	var invoice_val = $(this).text();
...
/*
$('.div_shower').on('submit','token_g3_patient',function(e){
alert('data is ' + data);
$.post("dental_b/", $(this).serialize())
	 .done(function(data) {
	alert('data is ' + data);
	var x = data.split('#');
		if(x[0] == "good"){
	if(x[1] == "treatment-done-allocation"){
					element.closest('.div_shower').dialog('close');
		alert('closed3');
...
//	else{alert ('nnnnnnnnnnnnnnnnnnnnnn');}
	...
	if(action == "#patient-contacts"){
	...
	else if(action == "#treatment-done"){
			//alert('fff');
			if($(this).hasClass('token_g2_patient')){
				var show_allocation_dialo = true;
				
				//loop over the treatment notes and ensure the statsus is set corrcetly
				$( ".treat_note_text" ).each(function(index, value  ) {
				  
				   if(this.value != ''  ){
					   var jina = $(this).attr('name');
					   var jina2 = jina.split('note');
					   var treatment_row = jina2[1];
					   
					   //get status value
					   var select_name = 'status' + treatment_row + '';
					   var status_value = $('select[name=' + select_name + ']').children("option:selected").text();
					   if(status_value == 'Not Started'){
						   alert('ERROR: You have enterd a treatment note for treatment ' + treatment_row + ' but not set the status to Partially Done or Done' );
						   all_is_good =  false;
		...
		if(all_is_good){
		 ...
		 var n = $( ".div_shower44" ).length;
		 if(n > 0){$( ".div_shower44" ).remove();}
			$(this).find('.submit_tdone').css({display: "none"});	
		// $('.mpesa_number').show();
		//$('.visa_number').css({display: "none"});	
		//$(this).find('.submit_tdone').remove();//$(this).submit(false);//prevent multi[ple insertions for new invs
		}
	}		
	else if(action == "invoice_payments_report"){
		$('.insurer_payment_datec').empty();
				$('.invoice_date').empty();
		$('.invoice_payment_content').empty().html('<img class=loader_gif src="dental_b/ajax_loader_for_main_page.gif" />');
	}
	else if(action == "insurnace_payment_date"){
		$('.insurer_payment_datec').empty();
				$('.insurer_payment_date').empty();
		$('.invoice_payment_content').empty().html('<img class=loader_gif src="dental_b/ajax_loader_for_main_page.gif" />');
	}	
	else if(action == "cash_balance_report"){
		$('.cash_balance_content').empty().html('<img class=loader_gif src="dental_b/ajax_loader_for_main_page.gif" />');
	}
	if(all_is_good){
	...
	/*$('.show_loader').dialog({
		...
		//alert('serial is ' + $(this).serialize());
	...
	//alert('data is ' + data);
		...
		if( r1.length == 2){
			...
			$('.insurer_payment_datec').empty();
				$('.invoice_date').empty();
			$('.invoice_payment_content').empty().html(data);
		}
		else if(action == "insurnace_payment_date"){
				$('.insurer_payment_datec').empty();
				$('.insurer_payment_date').empty();
			...
			$('.cash_balance_content').empty().html(data);
		}
	//	$('.show_loader').dialog('close');
		...
		//alert('action is ' + action);
			...
			if(n > 0){$( ".div_shower44" ).remove();}
				...
				if(show_allocation_dialo == true){
					//var show_allocation_dialo = true;
					//alert ('ttttttttttttttttyyyyyyyyyyyyyyyyyyyyyyyyyy');
					var show_allocation_dialog = 'yes';
					//alert(invoice_disp_num);
					var width_x = 800;//$('.div_shower').parent().width();
					//alert(width_x);
					var height_y = 120;//$("#employer_form_div").height();
					//alert(' width is ' + width_x + ' height is ' + height_y);
					$(".div_shower").empty().css('backgroundColor',' #15212F').dialog({
					title: 'TREATMENT STATUS ',
					height: height_y,
					width: width_x,
					modal: true}).load('dental_b/', {'show_allocation_dialog': show_allocation_dialog });
				
				}//end if
			}

			else if(x[1] == "treatment-done-allocation"){
			...
			//	$('#'+ tab_id).load("treatment-done/index.php");
			...
			else if(x[1] == "return_to_allocation"){
					$('.ui-icon-closethick').trigger('click');
					window.location = "?id=allocate-patients";
                
			}
			else if(x[1] == "on-examination"){
				$('#'+ tab_id).load("examination/index.php");
			...
			else if(x[1] == "test_emails"){
				window.location = "?id=email-credentials" ;
				...
				$('#'+ tab_id).load("treatment-plan/index.php");
				...
				if(action == "create_school_holiday_remider"){
					$('.create_appointent_reminder, .view_appointment_reminder').find('input, select, textarea').val('');		
					 $('.appointment_type').val('');		
						...
						$('.create_appointent_reminder, .view_appointment_reminder').slideUp('fast');		
				}//end else		
		}//end good if
						
		else if(x[0] == "bad"){
			if(x[1] == "tdone_admin_pass" && (action == "#treatment-done" || action == "#treatment-plan")){
				//alert('dddd');
				//if(){
					//alert('cccc');
					$('.admin_password_invoice_daily_limit').show();
					$('.submit_tdone').show();
					$('.feedback').empty()
			...
			else if(x[1] == "insurer_changed"){
				//$('.div_shower31').dialog().close();
				var width_x = $('.div_shower31').parent().width();
				var height_y = 580;//$("#employer_form_div").height();
...
$('#tdone_invoice_admin_approve').on('submit', '.patient_form_td', function(e){
	//alert('ff');
	var element = $(this);
	e.preventDefault();
	$.post("dental_b/", $(this).serialize())
	 .done(function(data) {
		...
		var x = data.split('#');
		if(x[0] == "good"){
			if(x[1] == "tdone_invoice_admin_approval"){
				var tdone_admin_invoice_approve1='yes';
				$('#tdone_invoice_admin_approve').load('dental_b/', {'tdone_admin_invoice_approve1': tdone_admin_invoice_approve1 });
			}
		}
		else if(x[0] == "bad"){
			if(x[1] == "tdone_invoice_admin_approval"){
...
//show transfer list for allocation surgeries
$('.hold_finish').change(function(){
	//var element = $(this);
	var val1 = $(this).val();
	//alert('d');
	if(val1 == 'transfer'){
		...
		$(this).next().next().next().children('select').val('');
		$(this).next().next().next().slideUp('fast');
	}
});

//select action in invoice from partially authorised invoices approve
$('.admin_inv_action').change(function(){
	//var element = $(this);
	var val1 = $(this).val();
	//alert('d');
	if(val1 == 'reply'){
...
//select action in invoice admin approve
$('#tdone_invoice_admin_approve').on('change', '.admin_inv_action', function(){
	//var element = $(this);
	var val1 = $(this).val();
	//alert('d');
	if(val1 == 'reply'){
...
$(' #cadcam_tabs4, #appointment_divr2, #append_invoice, .pt_contact_shower,#pt_contact_shower, #cadcam_tabs, #cadcam_tabs3, #ins_price_ed, .dialog_with_tab ,.div_shower44,  #appointment_div2,  #edit_waiting_list, .procedure_container2').off().on('submit', '.patient_form', function(e){
	//alert('oo1x');
...
//	else{alert ('nnnnnnnnnnnnnnnnnnnnnn');}
	...
	if(action == "#append_invoice"){
		...
		$('.drug_quantity').prop('disabled', false);//enale disabled inputs so that value can be sent to submission
		//alert('rrr');
	}		
	...
	//alert('11 ' + data);
		...
		//alert(x[2] + ' -- ' + x[3]);
			...
			//	window.location = "?id=edit-treatment-plan";
			...
			else if(x[1] == "appointment_form_tdone"){
				/*var lab_request = 'yes';
				$('.div_shower44').load('dental_b/', {'lab_request': lab_request });//enale disabled inputs so that value can be sent to submission
				$('.div_shower44').animate({scrollTop: '0px'}, 0);							
				$('html, body').animate({scrollTop: '0px'}, 0);	*/
				$('.feedback').empty()
							...
							.append(x[2])
							.show();
				var tdone_appointment_date = $('.tdone_appointment_date').val();
				var tdone_appointment_date2 = $('.tdone_appointment_date').val();
				$('#tdone_appointment_times').show().load('dental_b/', {'tdone_appointment_date': tdone_appointment_date });			
				$('.show_current_appointments').show().load('dental_b/', {'tdone_appointment_date2': tdone_appointment_date2 });
			}
			else if(x[1] == "appointment_for_school_holiday"){
				/*  disbale select and put appt date 	*/
				$('.div_shower44').dialog('close'); 
				$('.current_td').text(x[2]);
				...
				//alert('rrrr');
				//element.closest('form').parent('close');
			//	element.closest('div').dialog('close');
				//$('#pt_contact_shower').dialog("close");
				$('#pt_contact_shower').remove();
				var get_fam = 'yes';
				$('#family_div').load('dental_b/', {'get_fam': get_fam });
			}					
			else if(x[1] == "loyalty_points"){
			...
			//	location.reload();
				...
				var edit_invoice = x[3];
				$('.div_shower2').load('dental_b/', {'edit_invoice': edit_invoice },function () {
			...
			if(action == "#treatment-plan"){
				...
				element.find("input[type=text]").val("");
						...
						//$('.holiday_class, .appointment_type_div').slideUp('fast');
				}//end else		
		}//end good if
		...
		//	else{
			...
			if(typeof x2[1] !== 'undefined'){
				//alert('inside ' + x2[1]);
				//var width_x = $('.get_width').show().width();
				$(' #pt_contact_shower #imwe_family').html(x2[1]);
...
$("a.link_clor" ).on( "mouseenter", function() {
 ...
 //$('.div_shower44').on('change','.appointment_type',function(){
$('.appointment_type').change(function(){
	var val= $(this).val();
	if(val == 'view_appointment_reminder'){
		$('.create_appointent_reminder').slideUp('fast');
		$('.view_appointment_reminder').slideDown('fast');
	}
	else if(val == 'create_appointent_reminder'){
		$('.view_appointment_reminder').slideUp('fast');
		$('.create_appointent_reminder').slideDown('fast');
 ...
 //this will edit insurer  procedures
 $('.ins_procedure_edit').click(function(){
	//alert('ff');
	var edit_insured_procedure = $(this).prev().val();
	//alert(invoice_disp_num);
		...
		title: 'INSURED PROCEDURES',
		height: height_y,
		width: width_x,
		modal: true}).load('dental_b/', {'edit_insured_procedure': edit_insured_procedure });
 });
...
//display invoice added dynamically 
 ...
 //this will display an invoice
 $('.invoice_no_link').click(function(e){
 e.preventDefault();
	var invoice_disp_num = $(this).text();
	//alert('88 ' + invoice_disp_num);
		var width_x = $('.div_shower2').parent().width();
		var height_y = 580;//$("#employer_form_div").height();
		//alert(' width is ' + width_x + ' height is ' + height_y);
		$(".div_shower2").empty().css('backgroundColor',' #15212F').dialog({
 ...
 //display invoice
  $('.invoice_payment_content').on('click','.invoice_no_link',function(e){
	e.preventDefault();
	var invoice_disp_num = $(this).text();
	//alert('88 ' + invoice_disp_num);
		var width_x = $('.div_shower2').parent().width();
 ...
 $('.invoice_no').click(function(){
  ...
  //this will show partiall payments for invoices
 $('.balance_payment').click(function(e){
	//alert('ff');
	e.preventDefault();
	var partiall_payment = $(this).attr('href');
	//alert(partiall_payment);
		var width_x = $('.div_shower2').parent().width();
		var height_y = 580;//$("#employer_form_div").height();
		//alert(' width is ' + width_x + ' height is ' + height_y); .css('backgroundColor',' #15212F').
		$(".div_shower2").empty().css('backgroundColor',' #15212F').dialog({
		...
		height: height_y,
		width: width_x,
		modal: true}).load('dental_b/', {'partiall_payment': partiall_payment });
 }); 
 ...
 //$('.show_prescription').click(function(){
	...
	//alert(invoice_disp_num);
		...
		var height_y = 580;//$("#employer_form_div").height();
		//alert(' width is ' + width_x + ' height is ' + height_y);
		//if( $( ".dialog_with_tab" ).dialog( "isOpen" ) === true ){alert('open');}
		//else{alert('closed');}
		//$('.dialog_with_tab').dialog('close');
		$( ".dialog_with_tab" ).each(function( index ) {
...
$(document.body).on('change', '.dialog_form  #insured_yes_no', function(e){  
	...
	if(val == 'NO'){
		...
		var get_cash_for_non_insured = 'yes';
		var element = $(this).closest('form').find('.edit_corporate_employer');
		var form_data = {get_cash_for_non_insured: get_cash_for_non_insured}
		$.ajax({
		type: "POST",
		url: "dental_b/",
		data: form_data,
		...
		//$(this).closest('form').find('.edit_corporate_employer').val('').prop('disabled', false);
	}
	else if(val == 'YES'){
 ...
 //show reason for suspension of cover
 $(document.body).on('change', '.dialog_form  #suspend_cover', function(e){  
	var val= $(this).val();
	if(val == 'No'){
		$(this).closest('form').find('.suspend_reason_text').val('');
		$(this).closest('form').find('.suspend_reason_class').slideUp('fast');;
	}
	else if(val == 'Yes'){
		//$(this).closest('form').find('.suspend_reason_text').val('');
		$(this).closest('form').find('.suspend_reason_class').slideDown('fast');;
	}
 });
 
 //submit a form from a dialog box via jquery
$(document.body).on('submit', '.dialog_form2', function(e){
	...
	$.post("dental_b/", $(this).serialize())
	 ...
	 //alert(data);
		...
		$('<div class=feedback_dialog ></div>').insertBefore(element);
		var x = data.split('#');
		//alert(data);
		if(x[0] == "good"){
...
//$('.dialog_form').submit(function(e){
	...
	//alert('serialize ni 333 ' + $(this).serialize());
	
	$.post("dental_b/", $(this).serialize())
	 .done(function(data) {
	// alert('1618 data is ' + data);
		//$('.show_loader').dialog('close');
		if(action!='new_employer_action'){$('.feedback_dialog').remove();}
		$('<div class=feedback_dialog ></div>').insertBefore("#edit_covered_procedure");
		
		var x = data.split('#');
		//alert(data);
		if(x[0] == "good"){
			if(action=='new_employer_action'){
				//$("#employer_form_div").load('dental_b/', {'edit_corporate': edit_corporate });
		...
		//		}//end else		
			...
			if(action=='new_employer_action'){
				...
				if(x[1] == "patient_type_changed"){
					$('#edit_ins_cover').find('.corprate_swap_password').remove();
					$('.feedback_dialog').empty()
										.removeClass('success_response hide_element')
										.addClass('error_response')
										.append(x[2])
										.show();
					//remove submit button to avoid confusion
					$('.submit').remove();
...
//this is for booking appointment in tdone
$('.tdone_appointment').click(function(e){
	e.preventDefault();
	var tdone_appointment= 'yes';
	var width_x = $(this).parent().width();
	var height_y = 780;
	$(".div_shower44").empty().css('backgroundColor',' #15212F').dialog({
		...
		height: height_y,
		width: width_x,
		modal: true}).load('dental_b/', {'tdone_appointment': tdone_appointment });
	
});

$('.div_shower44').on('change','.tdone_appointment_date',function(){
	var tdone_appointment_date = $(this).val();
	var tdone_appointment_date2 = $(this).val();
	 
	if(tdone_appointment_date == ''){alert('Please specify the appointment date');}
	else{
		$('#tdone_appointment_times').show().load('dental_b/', {'tdone_appointment_date': tdone_appointment_date });
		//get appointments for that doctor on that day
		$('.show_current_appointments').show().load('dental_b/', {'tdone_appointment_date2': tdone_appointment_date2 });
	}
});		

//this is for booking appointment in school holidays
$('.set_appointment_status2').change(function(e){
	e.preventDefault();
	var tdone_appointment3= $(this).val();
	var n=tdone_appointment3.split("tengeneza");
	if(n.length == 2){
		//remove current_td from anyone who may have it
		$('.current_td').removeClass('current_td');
		$('.current_select').removeClass('current_select');
		//$(this).parent().parent().parent().find('current_td').removeClass('current_td');
		//put class in next td and use it to put new appt date
		$(this).parent().next().addClass('current_td');
		$(this).addClass('current_select');
		var tdone_appointment3= n[1];
		...
		$(".div_shower44").empty().css('backgroundColor',' #15212F').dialog({
			...
			height: height_y,
			width: width_x,
			modal: true}).load('dental_b/', {'tdone_appointment3': tdone_appointment3 });
	}
});

$('.div_shower44').on('change','.tdone_appointment_date3',function(){
	var tdone_appointment_date5 = $(this).val();
	var tdone_appointment_date4 = $(this).val();
	//alert(tdone_appointment_date2);
	if(tdone_appointment_date5 == ''){alert('Please specify the appointment date');}
	else{
		$('#tdone_appointment_times').show().load('dental_b/', {'tdone_appointment_date5': tdone_appointment_date5 });
		//get appointments for that doctor on that day
		$('.show_current_appointments').show().load('dental_b/', {'tdone_appointment_date4': tdone_appointment_date4 });
	}
});

$('.div_shower44').on('click','.toggle_appointment_form',function(){
	//alert('c');
	$(this).next().toggle();
});	
...
$('.prescribe').click(function(e){
	...
	//var width_x = $(this).parent().width();
	var width_x = $('.div_shower44').parent().width();
	var height_y = 780;
	//was $(".div_shower44")
	$(".div_shower44").empty().css('backgroundColor',' #15212F').dialog({
		title: 'Prescriptions',
...
//this is for showing tplan details in tplan visits report
$('.tplan_history').click(function(e){
	e.preventDefault();
	var tplan_history= $(this).text()
	var width_x = $(this).parent().parent().width();
	//var width_x = $('.div_shower').parent().width();
	var height_y = 780;
	$(".div_shower44").empty().css({'backgroundColor':'#15212F',
		...
		height: height_y,
		width: width_x,
		modal: true}).load('dental_b/', {'tplan_history': tplan_history });
	//alert('token is ' + token);
});
...
//this is for showing statemtn with swapped records
$('.pt_statement_swapped').click(function(e){
	e.preventDefault();
	//alert('ffff');
	var pt_statement_swapped= 'yes';
	var width_x = $(this).parent().width();
	var height_y = 780;
	$(".div_shower44").empty().css('backgroundColor',' #15212F').dialog({
		title: 'Statement',
		height: height_y,
		width: width_x,
		modal: true}).load('dental_b/', {'pt_statement_swapped': pt_statement_swapped });
	//alert('token is ' + token);
});

//this is for showing statemtn with swapped records for drugs sold report
$('.pt_statement_swapped2').click(function(e){
	e.preventDefault();
	//alert('ffff');
...
//show pt statement in points redeem report
$('.pt_statement_points_redem').click(function(e){
	e.preventDefault();
	var pt_statement_a = $(this).prev().val();
	var width_x = $('.waiver_table_row2').parent().width();
	if (width_x <= 0 ){width_x = $(this).parent().parent().width();}
	var height_y = 780;
	//alert('token is ' + pt_statement_a);
	$(".div_shower44").empty().css('backgroundColor',' #15212F').dialog({
		title: 'Statement',
		height: height_y,
		width: width_x,
...
//this is for showing pt balance from cash balances
//$('.pt_statement_a').click(function(e){
$('.cash_balance_content').on('click', '.pt_statement_a', function(e){
	e.preventDefault();
	var pt_statement_a = $(this).prev().val();
	var width_x = $('.waiver_table_row2').parent().width();
	if (width_x <= 0 ){width_x = $(this).parent().parent().width();}
...
$('.cash_balance_content').on('click', '.check_all_email', function(e){
...
$('.div_shower44').on('blur', '.drug_quantity', function(){

		if( $(this).val() !=''){
			if (IsNumeric_jq($(this).val())){
			//get unit price
			var unit_price = $(this).parent().next().text();
			unit_price = remove_commas(unit_price);
			unit_price =  unit_price.replace(/@/g, '');
				if (IsNumeric_jq(unit_price)){
					price = unit_price * $(this).val();
					price=addCommas(price);
					$(this).parent().next().next().children('.drug_price').val(price);
				}
...
//$('.pay_method').change(function(){
	...
	//this will ensure that in tplan only aviallable fields are not disabled
			...
			$(this).parent().next().next().next().children('.drug_quantity').val('');		
			$(this).parent().next().next().next().next().html('&nbsp;');			
			$(this).parent().next().next().next().next().next().children('.drug_price').val('');			
			
		if( $(this).val() !=''){
			//get stock availlable
			var drug_id_quantity = $(this).val();
			var form_data = {drug_id_quantity: drug_id_quantity}
			element = $(this);
			element.next().load('dental_b/', {'drug_id_quantity': drug_id_quantity });;
			$(this).parent().next().children('.drug_details').prop('disabled', false);		
			$(this).parent().next().next().children('.drug_presc_type').prop('disabled', false);
			$(this).parent().next().next().next().children('.drug_price').prop('disabled', true);					
		}
...
//$('.pay_method').change(function(){
		...
		//alert(selec_text);
		$(this).parent().next().children('.drug_quantity').val('');	
		...
		if( selec_text == 'Prescribe'){
			...
			$(this).parent().next().children('.drug_quantity').prop('disabled', true);		
			//$(this).parent().next().next().children('.drug_unit_price').val('');	
			$(this).parent().next().next().next().children('.drug_price').prop('disabled', true);			
		}
		else if( selec_text == 'Sell'){
			$(this).parent().next().children('.drug_quantity').prop('disabled', false);	
			$(this).parent().next().next().next().children('.drug_price').prop('disabled', false);					
			//get drug price
			...
			//alert('data is ' + data);
				...
				element.parent().next().next().html(data);	
			},
			complete: function() {
			}
			});
...
$('.div_shower44').off('click', '.add_drug');
$('.div_shower44').on('click', '.add_drug', function(e){
	...
	//$('.add_drug').click(function(){
		...
		//alert ('i is ' + i);
		//alert('ff');
		//alert('div ni ' + $('.div_shower44').length + ' na button ni ' +$('.add_drug').length );
			var add_drug = 'yes';
...
//$('.new_family').click(function(){
 ...
 $('tr').hover(function(e){
		$(this).find('input, select').addClass('text_field_highlight'); 
		
	},
	function(e){
		$(this).find('input, select').removeClass('text_field_highlight');
		
	}
 );
 
 /*
 $('tr.initiator').on('mouseenter mouseleave', function(e) {
   // $('.receiver').trigger(e.type);
	$(this).find('td:first').trigger(e.type);
})*/

//  $('td').click(function(e){$(this).hover();});
/*
 $("td").hover(function() {
	//alert('rrr');
  $el = $(this);
  
  $el.parent().addClass("table_row_click_bg");
  ...
  if ($el.parent().has('td[rowspan]').length == 0)
      ...
      .find('td[rowspan]')
      .addClass("table_row_click_bg");

}, function() { 
      
  $el
    .parent()
    .removeClass("table_row_click_bg")
    .prevAll('tr:has(td[rowspan]):first')
    .find('td[rowspan]')
    .removeClass("table_row_click_bg");
 ...
 //ensure that links in allocation has same color as other text
 //var element = $('td.alloc_link_color');
 //$('td.alloc_link_color a').css('color',$(this).parent().prev().text().css("color"));
 $("table td.count_class").each(function(){
   var col = $(this).css('color');
   $(this).next().children('a').css('color', col);
    $(this).next().next().children('a').css('color', col);
});
...
$(document.body).on('change', '.drug_presc_type', function(){
	var selec = $(this).children("option:selected").text();
	if(selec == 'Sell'){
		$(this).parent().find('.commissioners_div').show();
	}
	else{
		$(this).parent().find('.commissioners_div').children("option:selected").removeAttr("selected");
		$(this).parent().find('.commissioners_div').css({display: "none"});	
	}
});		

//this will show payment action for payments
$('.payment_action').change(function(){
	var selec = $(this).val();
	if(selec != 'make_payment' && selec != 'cash_pledge'   ){
		$('.cash_pledge').css({display: "none"});	
		$('.make_payment').css({display: "none"});	
	}
	else if(selec == 'make_payment' ) {
        $('.cash_pledge').css({display: "none"});	
		$('.make_payment').show();
	}
	else if(selec == 'cash_pledge' ) {
		 $('.make_payment').css({display: "none"});	
		$('.cash_pledge').show();
...
$('.procedure_container2').on('click', '.add_new_procedure_edit_tplan_no_invoice2', function(){
	...
	var aliased_invoice = $(this).prev().prev('.aliased_invoice').val();
	//extra_procedure_invoice = aliased_invoice;
//	alert ('extra_procedure_tplan_no_invoice is ' + extra_procedure_tplan_no_invoice);
		var form_data = {extra_procedure_invoice: extra_procedure_invoice, aliased_invoice: aliased_invoice}
		element = $(this);
		$.ajax({
		type: "POST",
		url: "dental_b/",
...
//this is for getting aliases in tplan
$('.procedure_container').on('click', '.get_alias_button', function(){
	//get selected tplans to be aliaesd
	var element_sel = $(this).parent().prev().prev().attr('name');
	var element_par = $(this).parent().next();
	element_par.empty();
	$('.procedure_container .tplan_alias').each(function(){
			if($(this).prop('checked') == true){
				//get tplan name
				var tplan_name1 = $(this).parent().prev().prev().prev().children('.select_procedure');
				var tplan_name = tplan_name1.children("option:selected").text();
				var gap2 = $(this).val();
				var form_data = {gap2: gap2, element_sel:element_sel,tplan_name: tplan_name  }
					$.ajax({
						type: "POST",
						url: "dental_b/",
						data: form_data,
...
$('.patient_list_report').change(function(){
		...
		if( $(this).val() !='' && selec == 'registered' ){
			...
			$('.loyalty_div').slideUp('fast');	
		}
		/*else if( $(this).val() !='' && selec == 'last_seen'){
			$('.registration_div').css({display: "none"}).slideUp('fast');
			$('.last_seen_div').slideDown('fast');	
			$('.loyalty_div').slideDown('fast');				
		}*/
		else if( $(this).val() !='' && selec == 'redeem_points'){
			$('.loyalty_div').slideDown('fast');
			//$('.last_seen_div').slideDown('fast');	
			$('.registration_div').slideUp('fast');				
		}
...
//alert('998');
		...
		$(this).parent().next().next().next().children('.tplan_alias').attr('checked', false);
		get_treatment_plan_total_cost2();
	//this will ensure that in tplan only aviallable fields are not disabled
		...
		if( $(this).val() !='' ){
			...
			$(this).parent().next().next().next().children('.tplan_alias').prop('disabled', false);	 
			...
			$(this).parent().next().next().next().children('.tplan_alias').prop('disabled', true);
		}
	...
	//check if the procedure is covered by insurance
		var check_procedure_payment_method = add_procedure2 ;
		var form_data = {check_procedure_payment_method: check_procedure_payment_method}
		pay_method_select = $(this).parent().next().next().next().children('.pay_method');
		...
		success: function(data) {
		 ...
		 else if(x2[0] == 'covered' && x2[1] != ''){
			 var insurance_found = false;
			$('option ' ,  pay_method_select).each(function(){
				if($(this).text() == 'Insurance'){	
					 insurance_found = true;
				}
			});
			if(!insurance_found){pay_method_select.append(x2[1]);}
		 }
		},
		complete: function() {
		}
...
/*
//this is for showinging aliases to be selected
$('.procedure_container').on('change', '.tplan_alias', function(e){
	var name1 = $(this).attr('name');
	var one_checked = false;
	//var element = $(this).parent().prev().prev().prev().find('.get_alias_button');
	//var nn = $(this).parent().prev().prev().prev().find('.get_alias_button').attr('name');
	//alert('nn is ' + nn);
		$('.procedure_container .tplan_alias').each(function(){
			if($(this).prop('disabled') == true){}
			else if($(this).prop('checked') == true){
			//	$('.procedure_container .get_alias_button').css({visibility: "visible"});//slideDown('fast');
				
				$(this).parent().prev().prev().prev().find('.get_alias_button').css({visibility: "hidden"});
				one_checked= true;
			//	alert('ffff');
			//	return false;
			}
			else{
				$(this).parent().prev().prev().prev().find('.get_alias_button').css({visibility: "visible"});//slideDown('fast');}
			}
		});
		
		if(one_checked == false){
			alert('ggg');
			$('.procedure_container .get_alias_button').css({visibility: "hidden"});//.slideUp('fast');
		}
	});
*/
...
element = $(this);
	...
	//check if the procedure is covered by insurance
		var check_procedure_payment_method = add_procedure ;
		var form_data = {check_procedure_payment_method: check_procedure_payment_method}
		//pay_method_select = $(this).parent().next().next().children('.pay_method');
		$.ajax({
		type: "POST",
...
//this will show form for adding patient refferes
 ...
 $('.edit_corporate').click(function(){
	var edit_corporate2 = $(this).next().val();
		var width_x = $(this).parent().parent().width();
		var height_y = 700;//$("#employer_form_div").height();
		//alert(' width is ' + width_x + ' height is ' + height_y);
		$("#edit_ins_cover").empty().dialog({
		close: function() {
  ...
  //this will xray refferer 
 $('.edit_xray_referer').click(function(){
	var edit_xray_referer = $(this).next().val();
		var width_x = $(this).parent().parent().width();
		var height_y = 240;//$("#employer_form_div").height();
		//alert(' width is ' + width_x + ' height is ' + height_y);
		$("#edit_xray_ref_inner").empty().dialog({
		close: function() {
		   window.location = "?id=xray-referrer";
		},
		title: 'Edit X-Ray Refferer',
		height: height_y,
		width: width_x,
		modal: true}).load('dental_b/', {'edit_xray_referer': edit_xray_referer });
 });
...
$('#appointment_divr2').on('change','.appointment_doctor',function(){
 ...
 $('.set_appointment_status').change(function(){
	var selec_text = $(this).children("option:selected").text(); 
	//alert(selec_text);
	if(selec_text == 'RE-APPOINTED' || selec_text == 'Create Appointment'){
...
//create holiday appointment for shcols
 $('.appoint_td').on('click','.new_appointment_button',function(){
 //$('.create_appointment').click(function(){
	//alert('fff');
	var selec_text = $(this).parent().prev().children('select').children("option:selected").text(); 
	var appointment_id2 = $(this).parent().prev().children('select').val();
	$('.re_appoint_td').removeClass('current_reappoint');
	$(this).parent().addClass('current_reappoint');
	var form_data = {appointment_id2: appointment_id2}
	$.ajax({
		type: "POST",
		url: "dental_b/",
		data: form_data,
...
//this will test all email accoubts
	$('.test_all_emails').click(function(){
	//alert('ff');<img src="dental_b/ajax-loader-spinner.gif" />
	$(this).parent().next().empty().append('<label class=label>Testing authentication please wait</label> <img src="dental_b/ajax-loader-spinner.gif" />');
	var test_all_email_accounts = 'test_all_email_accounts';
	$(this).parent().next().load('dental_b/', {'test_all_email_accounts': test_all_email_accounts });
...
$(".div_shower2 , .div_shower44 , .div_shower").on('click','.printment',function(event){
 ...
 //this will show invoice edit history
$(".div_shower2 ").on('click','.show_invoice_history',function(event){
	
	$(this).parent().next().children('table').slideDown();
...
//this will select the insurance payment report type
$('#invoice_pay_report_type').change(function(){
		var pay_mode = $(this).val();
		if(pay_mode == ''){
			$('.insurer_payment_date').slideUp('fast');
			$('.invoice_date').slideUp('fast');	
		}
		else if(pay_mode == 'invoice'){
			$('.invoice_date').slideDown('fast');
			$('.insurer_payment_date').slideUp('fast');	
		}
		else if(pay_mode == 'insurer'){
			$('.invoice_date').slideUp('fast');
			$('.insurer_payment_date').slideDown('fast');	
		}		
});	
...
//this will show the action in cash pledges report
$('.cp_action').change(function(){
		var val1 = $(this).val();
		if(val1 == ''){
			$(this).parent().parent().find('.new_pledge_date1a').slideUp('fast');
			//$('.serach_by_ins').slideUp('fast');	
		}
		else if(val1 == 'delete1'){
			$(this).parent().parent().find('.new_pledge_date1a').slideUp('fast');
		}
		else if(val1 == 'new_pledge1'){
			$(this).parent().parent().find('.new_pledge_date1a').slideDown('fast');
		}		
});
