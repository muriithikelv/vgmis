...
<div class='grid-container '>
	...
	<?php //include  '../../dental_includes/response.php'; 
			...
			if(isset($_SESSION['tplan_id']) and $_SESSION['tplan_id']!=''){
				  $follow_up ="<a href='' class='link_color follow_up' >Follow Up</a> | ";
				  $cadcam ="|  <a href='' class='link_color tdone-cadcam' >CADCAM</a>  ";
				  $lab_request ="<a href='' class='link_color  lab_request' >Lab Request</a>   ";
				 }
		...
		$follow_up $lab_request 
		 <a href='' class='link_color pt_statement_swapped' >Statement</a> |  <a href='' class='link_color treatment_history' >Treatment History</a> | 
		 <a href='' class='link_color tdone-invoice' >Invoices </a> $cadcam ";
			//echo "<div class='clear'></div><br>";
			echo "<div class='grid-100 label remove_left_padding'>Patient Tag: $_SESSION[tag]</div>";
			}
 ...
 if(isset($_SESSION['tplan_id']) and $_SESSION['tplan_id']!=''){
		...
		$placeholders2[':tplan_id']=$_SESSION['tplan_id'];
		...
		<form action="#treatment-done" method="POST"  name="" id="" class="patient_form2 token_g2_patient">
		...
		foreach($s as $row){
			...
			echo "<tr class='has_css_div $class_ts'>";?>
				...
				echo "<td class='td_div_holder '>";	
					...
					if($row['status']!='Done'){
							...
							<?php echo "	<div class='tplan_table_row2 $class_ts'>"; ?>
									...
									<div class=treat_notes><?php echo "<textarea  class='treat_note_text' rows='' name=$note ></textarea>"; ?></div>
								</div>
							</div>	 
							<?php
					} ?>
				...
				<td><?php
					...
					elseif($row['invoice_number']=='' and $row['pay_type']=="Insurance" and !$_SESSION['ins_suspend']){
 ...
 exit;
		...
		*/
		...
		<div class='grid-100 toa_left_padding'><br><input type=button class='button_style printment' value='Print Treatment Plan' /></div>
		<div>
		...
		foreach($s as $row){
			if($i == 0){
				...
				?>
				<div class='grid-10 toa_left_padding label no_print'>Complaint:</div><div class='grid-90  label no_print'><?php echo "$complaint"; ?></div>
				<div class='grid-10 toa_left_padding label no_print'>Diagnosis:</div><div class='grid-90  label no_print'><?php echo "$diagnosis"; ?></div>
				<table class='table_border normal_table'><caption>UNFINISHED/UNINVOICED TREATMENT PLANS</caption>
				<tr>
					<th class='tplan_created white_text'>Date<br>Created</th><th class='tplan_id  white_text'>Treatment<br>Plan<br>No.</th>
					<th class='tplan_procedure white_text'>Procedure</th><th class='tplan_status white_text'>Status</th>
					<th class='tplan_last_seen white_text'>Last<br>Seen</th><th class='tplan_select white_text'>Select</th>
				</tr>
			<?php
				$i++;
			...
			//get visits planned fort he tplan
				$visits='';
				$sql2=$error2=$s2='';$placeholders2=array();
				$sql2="select visits_planned, visits_remaining from tplan_visits where tplan_id=:tplan_id";
				$placeholders2[':tplan_id']=$row['tplan_id'];
				$error2="Unable to get pending visits";
				$s2 = select_sql($sql2, $placeholders2, $error2, $pdo);
				foreach($s2 as $row2){
					$visits_planned = html($row2['visits_planned']);
					$visits_remaining = html($row2['visits_remaining']);
					//$visits=$visits_planned - $visits_remaining;
					$visits="<br>$visits_remaining of $visits_planned visits remaining";
			...
			echo "<tr >";
				...
				$error2="Unable to get unfinished treatment plan procedure";
				$s2 = select_sql($sql2, $placeholders2, $error2, $pdo);	
				$row_count = $s2->rowCount();
				echo "<td class=tplan_created rowspan=$row_count >";htmlout($row['when_added']); echo "</td>";//date created
				echo "<td  class='tplan_id' rowspan=$row_count >";htmlout($row['tplan_id']); echo "$visits </td>";//tplan id
				$submit_printed=false;
				foreach($s2 as $row2){ 
					...
					<!--<div class=tplan_procedure_table>-->
						...
						<?php  ?>
							...
							echo "</td>";
							if(!$submit_printed ){//show submit button
								if($swapped==''){
									echo "<td class=tplan_select rowspan=$row_count>";
									?>	
								...
								elseif($swapped!=''){echo "<td  rowspan=$row_count></tr>";}
								$submit_printed=true;
							}
							else{echo "</tr>";}
