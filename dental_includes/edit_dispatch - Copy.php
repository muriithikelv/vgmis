...
function edit_dispacth($pdo,$encrypt,$disp_num,$dipatcher_name,$title,$when_added,$insurer_id,$insurer_name){
	...
	foreach($s as $row){
		...
		$sql2="SELECT sum(amount) from payments where invoice_id=:invoice_id group by invoice_id";
		...
		elseif($paid > 0){
			...
			if($row['amount_authorised'] == $paid){$invoice_status='Paid';}
			elseif($row['amount_authorised'] > $paid){$invoice_status='Partially Paid';}
			elseif($row['amount_authorised'] < $paid){$invoice_status='ERROR';}
		}
