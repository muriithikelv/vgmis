...
<?php
	...
	$sql=$error=$s='';$placeholders=array();
	$sql="select when_added,description,reminder_date,id from reminders where created_by=:created_by and approved='no' and 
			reminder_date <= now()";
	...
	if($s->rowCount() > 0){
		...
		<tr><th class=rm_count></th><th class=rm_date>DATE CREATED</th><th class=rm_details>REMINDER</th>
		...
		foreach($s as $row){
			...
			echo "<tr><td >$count</td><td >$date_created</td><td >$reminder</td><td >$date_due</td>
