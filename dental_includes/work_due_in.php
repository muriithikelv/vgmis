...
if(!userIsLoggedIn() or !userHasRole($pdo,31)){exit;}
echo "<div class='grid_12 page_heading'>LAB WORK DUE IN</div>";
?>
<div class=grid-container>
<?php 
//receive labs
if(isset($_POST['token_work_due_in2']) and $_POST['token_work_due_in2']!='' and $_POST['token_work_due_in2']==$_SESSION['token_work_due_in2'] ){
	$_SESSION['token_work_due_in2']='';
	//first receive lab work
	$n=0;
	...
	try{
			...
			while($i < $n){
				...
				$sql = "update labs set date_returned=now() where lab_id=:lab_id";
			...
			if(isset($_POST['trays']) and $_POST['trays']!=''){
				//now receive trays
				$tray=$_POST['trays'];
				$n=count($tray);
				$i=0;			
			...
			if($tx_result){echo "<div class='grid-100 feedback success_response'>Labs Received</div>";}
			elseif(!$tx_result){echo "<div class='grid-100 feedback error_response'>Unable to receive labs</div>";}
		}
		catch (PDOException $e)
		{
		$pdo->rollBack();
		echo "<div class='grid-100 feedback error_response'>Unable to receive labs</div>";
		}
}


	//get work pending collection by technixian
	if(isset($_POST['technician']) and $_POST['technician']!='' and isset($_POST['token_work_due_in1']) and $_POST['token_work_due_in1']!='' 
	and $_POST['token_work_due_in1']==$_SESSION['token_work_due_in1']){ ?>
	<form action="" method="POST" enctype="" name="" id="">

	
			
	<div class='grid-15'>
					<?php $token = form_token(); $_SESSION['token_work_due_in1'] = "$token";  ?>
	<input type="hidden" name="token_work_due_in1"  value="<?php echo $_SESSION['token_work_due_in1']; ?>" />
	...
	<?php
		...
		a.doc_id=c.id  $criteria and date_returned is null $criteria_date order by a.lab_id";
		$error="Unable to get work due out";
		$s = select_sql($sql, $placeholders, $error, $pdo);	
		if($s->rowCount() > 0){
			$count=0;
			echo "<br><br><form action='' method='post' name='' id='' class=''>
			<table class='normal_table'><caption>Lab work due in for $tech_name</caption><thead>
			...
			<th class=lab_in_date>DATE <br>REQUIRED</th><th class=lab_in_tray>TRAYS RETURNED</th><th class=lab_in_finished>WORK FINISHED</th>
			</tr></thead><tbody>";
			foreach($s as $row){
				$count++;
				...
				echo "</td><td><input type=checkbox name=finished[] value='$val' /></td></tr>";

			}
			echo "</tbody></table>";
			echo "<br>";
			$token = form_token(); $_SESSION['token_work_due_in2'] = "$token";  
			echo "<input type=hidden name=token_work_due_in2  value='$_SESSION[token_work_due_in2]' /><input type=submit class='put_right' value='Submit' /></form>";
		}
